...
| Mac control app | v0.9: automatic first-run guide and local Chat live proof passed; project runner pending | The original 8B Granite default was kernel-OOM-killed before its context was bounded, and then generated only about 0.6 tokens/sec. A bounded VPS benchmark found Granite 4.1 3B at 14.9 tokens/sec and 3.9 GiB peak RSS; SmolLM3 3B reached 15.2 tokens/sec and 3.6 GiB. The installed v0.8 app then passed its exact Granite 3B setup proof (`READY.`, 13.7 generated tokens/sec) after 5 GiB disk and 6 GiB available-memory checks. v0.9 opens setup on first launch, uses one finite local-model chat with no planner/tools/build route, requires one confirmation before a paid plan and a second before an approved build. The actual bundled Granite Chat proof returned a clean one-sentence reply and left no model process; no coding CLI ran. A new project plan is recorded as a clean Git base, but its approved-build helper refuses until a project-specific runner/checks pass a real VPS proof. Native tool calling remains unproved: a bare llama.cpp prompt needs a model-specific tool template and parser. |
...
Build and prove a small project-specific runner/template on a prepared VPS:
it must carry its own checks, accept the clean plan from the Mac app, and allow
the second confirmation alone to start a real coding run. Record the exact
plan-and-run output before scheduling additional providers or a J07 rerun.
