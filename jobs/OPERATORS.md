...
| Mac control app | v0.7 resource-bound fix in progress | The first live Granite attempt proved its process was kernel-OOM-killed at 23.09 GiB RSS on a 23 GiB, no-swap VPS; no helper was enabled. v0.7 bounds the helper to a 4K context, small batches, and a 15-minute proof before repeating that user-approved run. The project helper remains plan-only and any offline draft still needs explicit approval. |
