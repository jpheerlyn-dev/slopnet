...
| Mac control app | v0.8 3B default live proof passed; first project pending | The original 8B Granite default was kernel-OOM-killed before its context was bounded, and then generated only about 0.6 tokens/sec. A bounded VPS benchmark found Granite 4.1 3B at 14.9 tokens/sec and 3.9 GiB peak RSS; SmolLM3 3B reached 15.2 tokens/sec and 3.6 GiB. The installed v0.8 app then passed its exact Granite 3B setup proof (`READY.`, 13.7 generated tokens/sec) after 5 GiB disk and 6 GiB available-memory checks. Granite 3B is the default because IBM documents its tool calling and it is Apache-2.0. Native tool calling remains unproved: a bare llama.cpp prompt needs a model-specific tool template and parser. The project helper remains plan-only and any draft still needs explicit approval. |
