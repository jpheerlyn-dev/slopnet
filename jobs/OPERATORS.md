...
| Mac control app | v0.7 local model proof passed; first project pending | The first Granite attempt was kernel-OOM-killed at 23.09 GiB RSS on a 23 GiB, no-swap VPS. v0.7 bounded it to a 4K context, small batches, and a timeout; the installed app then cached Granite and the same offline proof returned `READY` with exit 0. CPU-only generation is slow (about 0.6 tokens/sec), so its role stays a short optional request draft. The project helper remains plan-only and any draft still needs explicit approval. |
