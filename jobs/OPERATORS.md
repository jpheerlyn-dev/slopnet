...
| Mac control app | v0.8 3B default selected; live default proof pending | The original 8B Granite default was kernel-OOM-killed before its context was bounded, and then generated only about 0.6 tokens/sec. A bounded VPS benchmark found Granite 4.1 3B at 14.9 tokens/sec and 3.9 GiB peak RSS; SmolLM3 3B reached 15.2 tokens/sec and 3.6 GiB. Granite 3B is the new default because IBM documents its tool calling and it is Apache-2.0. The benchmark does not certify native tool calling: a bare llama.cpp prompt needs a model-specific tool template and parser. v0.8 reserves 5 GiB disk and 6 GiB available memory before its local-helper install. The project helper remains plan-only and any draft still needs explicit approval. |
