...
into it — none are required, the checks work bare, and each can be added
...
|---|---|---|---|
| **codebase-memory-mcp** (DeusData) | Whole-codebase structural memory over MCP — the agent *sees* the repo as a knowledge graph instead of grepping blind. Directly attacks "nothing is picked up unless an agent searches for it." | MIT | Run its installer (auto-configures Claude Code, Cursor and 40+ agents via `.mcp.json`); indexes live locally under `~/.cache/`. Read-only: it never edits your tree, so it needs no check of its own. |
| **Hermes Agent** (Nous Research) | A Claude-Code-class open runtime with sandboxed execution (local/Docker/SSH). Another worker the checks govern. | MIT | Reads `AGENTS.md` natively today. A dedicated adapter (prompt auto-logging + protected-path guard, like `adapters/claude-code/`) is the v0.2 item. |
...
| **StormCode** (the operator's own) | A deterministic multi-agent factory: planner → plan.yaml → parallel implementers in worktrees → tests-as-oracle → reviewer gate → merge. Routes roles to any provider (Anthropic / OpenAI-compatible / CLI agents). | private (operator's) | The factory to SlopNet's law: point its `gates.test_command` at `slopnet check --all && <your tests>` and its `protected_paths` at PROTECTED.txt, and every swarm task is SlopNet-governed automatically. |
...
get governed by the checks like any other agent; nothing in this table
